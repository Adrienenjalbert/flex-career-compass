...
import { CheckCircle2, XCircle, Lightbulb, Building2, ArrowRight, Mail, Download, Copy } from 'lucide-react';
...
const CoverLetterPage = () => {
  ...
  const template = templateSlug ? coverLetterTemplates[templateSlug] : null;

  if (!template) {
    return <Navigate to="/career-hub/cover-letters" replace />;
  }
  ...
  // Build cover letter with specific data
  const buildCoverLetterWithData = (data: Record<string, string>) => {
    let content = '';
  ...
  // Build cover letter with user's input
  const buildFullCoverLetter = () => buildCoverLetterWithData(variableValues);
  
  // Build cover letter with example data
  const buildExampleCoverLetter = () => buildCoverLetterWithData(template.exampleData);

  const copyFullCoverLetter = () => {
    navigator.clipboard.writeText(buildFullCoverLetter());
    toast.success('Full cover letter copied to clipboard!');
  };

  const downloadCoverLetter = () => {
    const blob = new Blob([buildFullCoverLetter()], { type: 'text/plain' });
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url;
    a.download = `${template.slug}-cover-letter.txt`;
    a.click();
  ...
  // Related templates
  const relatedTemplates = Object.values(coverLetterTemplates)
  ...
  return (
    <Layout>
      <Helmet>
        ...
        <link rel="canonical" href={`https://flex-career-compass.lovable.app/career-hub/cover-letters/${template.slug}`} />
      </Helmet>
      ...
      <div className="container mx-auto px-4 py-8 max-w-7xl">
          ...
          <div className="lg:w-80">
            <Card className="sticky top-4">
              ...
              <CardContent className="space-y-3">
                <Button onClick={copyFullCoverLetter} className="w-full" variant="default">
                  <Copy className="w-4 h-4 mr-2" />
                  Copy Full Letter
                </Button>
                <Button onClick={downloadCoverLetter} className="w-full" variant="outline">
          ...
          <Tabs defaultValue="example" className="mb-8">
            ...
            <TabsContent value="example" className="mt-6">
              <Card>
                ...
                <CardContent>
                  <div className="bg-white dark:bg-gray-900 border rounded-lg p-8 font-serif text-sm whitespace-pre-wrap min-h-[400px] leading-relaxed">
                    {buildExampleCoverLetter()}
                  </div>
            ...
            <TabsContent value="preview" className="mt-6">
              <Card>
                <CardHeader>
                  <div className="flex items-center justify-between">
                    <CardTitle>Your Cover Letter Preview</CardTitle>
                    <div className="flex gap-2">
                      <Button size="sm" variant="outline" onClick={copyFullCoverLetter}>
                  ...
                  <CardDescription>
                    Your completed cover letter based on the information you've entered
                  </CardDescription>
                </CardHeader>
                <CardContent>
                  <div className="bg-white dark:bg-gray-900 border rounded-lg p-8 font-serif text-sm whitespace-pre-wrap min-h-[400px] leading-relaxed">
                    {buildFullCoverLetter()}
                  </div>
                </CardContent>
              </Card>
            </TabsContent>
        ...
        <section className="mt-12">
          <h2 className="text-2xl font-bold mb-6">Other Cover Letter Templates</h2>
          <div className="grid md:grid-cols-3 gap-6">
            {relatedTemplates.map((related) => (
              <Link key={related.slug} to={`/career-hub/cover-letters/${related.slug}`}>
        ...
        <div className="mt-12 text-center bg-gradient-to-r from-primary/10 to-primary/5 rounded-2xl p-8">
          <h2 className="text-2xl font-bold mb-4">Put Your Cover Letter to Work</h2>
